...
export const TopScorerQuestion = {
    ...
    worldCupService.getMatches().forEach(match => {
        const year = match.tournament_name.replace(Constants.WORLD_CUP_REGEX, '');
    ...
    let maxGoals = 0;
    let explanationParams: Record<string, any> = { year };
        ...
        explanationParams = { ...explanationParams, player: correctAnswer, goals: maxGoals };
    //}
    ...
    return {
        ...
        explanation: t('questions.explanationTopScorer',explanationParams),
        difficulty: 'medium',
